...
static void internal_gpio_close_all(void)
{
	...
	for (uint_fast8_t pin = 0; pin < gpio_count; pin++, gpio++)
	{
		if (gpio->is_open)
		{
			...
			{
				ACC_LOG_ERROR("Expected to write %d bytes to gpio unexport, but wrote %d for gpio%" PRIuFAST8, (int)gpio_x_len,
				              (int)bytes_written, gpio->pin);
			}
...
static void unregister_isr(uint_fast8_t pin)
{
	...
	if (is_isr_registered(gpio))
	{
		...
		acc_os_thread_cleanup(gpio->thread_handle);
		gpio->thread_handle = NULL;
...
static bool register_isr(uint_fast8_t pin, acc_gpio_edge_t edge, acc_device_gpio_isr_t isr)
{
	...
	if (status)
	{
		...
		gpio->thread_handle = acc_os_thread_create(&wait_for_interrupts, gpio, "gpio_wfi");
		if (gpio->thread_handle == NULL)
		{
