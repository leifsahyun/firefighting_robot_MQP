...
typedef struct acc_app_integration_mutex
{
	uint_fast8_t    is_initialized;
	pthread_mutex_t mutex;
} acc_app_integration_mutex_s;

typedef struct acc_app_integration_semaphore
{
	uint_fast8_t is_initialized;
	sem_t        handle;
} acc_app_integration_semaphore_s;
...
typedef struct acc_app_integration_thread_handle
{
	pthread_t handle;
	thread_t  info;
} acc_app_integration_thread_handle_s;
...
static void acc_driver_os_sleep_ms(uint32_t time_msec)
{
	acc_driver_os_sleep_us(time_msec * 1000);
}
 ...
 */
static acc_app_integration_thread_id_t acc_driver_os_get_thread_id(void)
{
 ...
 */
static uint32_t acc_driver_os_get_time(void)
{
 ...
 */
static acc_app_integration_mutex_t acc_driver_os_mutex_create(void)
{
	acc_app_integration_mutex_t mutex = acc_os_mem_alloc(sizeof(*mutex));
...
static void acc_driver_os_mutex_destroy(acc_app_integration_mutex_t mutex)
{
 ...
 */
static void acc_driver_os_mutex_lock(acc_app_integration_mutex_t mutex)
{
 ...
 */
static void acc_driver_os_mutex_unlock(acc_app_integration_mutex_t mutex)
{
 ...
 */
static acc_app_integration_thread_handle_t acc_driver_os_thread_create(void (*func)(void *param), void *param, const char *name)
{
	int ret;

	acc_app_integration_thread_handle_t thread = acc_os_mem_alloc(sizeof(*thread));
 ...
 */
static void acc_driver_os_thread_cleanup(acc_app_integration_thread_handle_t thread)
{
...
static acc_app_integration_semaphore_t acc_driver_os_semaphore_create(void)
{
	acc_app_integration_semaphore_t sem = NULL;
...
static bool acc_driver_os_semaphore_wait(acc_app_integration_semaphore_t sem, uint16_t timeout_ms)
{
	struct timespec ts;
...
static void acc_driver_os_semaphore_signal(acc_app_integration_semaphore_t sem)
{
...
static void acc_driver_os_semaphore_signal_from_interrupt(acc_app_integration_semaphore_t sem)
{
	acc_os_semaphore_signal(sem);
}


static void acc_driver_os_semaphore_destroy(acc_app_integration_semaphore_t sem)
{
	if (sem != NULL && sem->is_initialized)
	{
...
void acc_driver_os_linux_register(void)
{
	...
	acc_device_os_sleep_ms_func                        = acc_driver_os_sleep_ms;
