...
// Public definitions
//-----------------------------
acc_detector_distance_basic_handle_t acc_detector_distance_basic_create(acc_sensor_id_t sensor_id, float range_start, float range_length)
{
	...
	acc_service_requested_start_set(envelope_configuration, range_start);
	acc_service_requested_length_set(envelope_configuration, range_length);
	acc_service_sensor_set(envelope_configuration, sensor_id);
	acc_service_receiver_gain_set(envelope_configuration, 0.82f);
	acc_service_hw_accelerated_average_samples_set(envelope_configuration, 7);
	...
	if (detector_handle != NULL)
	{
		...
		if (detector_handle->envelope_handle != NULL)
		{
			...
			detector_handle->distance_offset = envelope_metadata.start_m;
			detector_handle->distance_slope = envelope_metadata.length_m / (float)(envelope_metadata.data_length - 1);
...
bool handle_valid(acc_detector_distance_basic_handle_t detector_handle)
{
	...
	{
		ACC_LOG_ERROR("Invalid detector handle");
		valid = false;
	}
